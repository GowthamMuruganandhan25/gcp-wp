...
resource "google_compute_network" "vpc-net"{
  name="gowtham-terraform-vpc"
  auto_create_subnetworks = "false"
  routing_mode = "REGIONAL"
}
resource "google_compute_subnetwork" "vpc-subnet" {
  name = "gowtham-terraform-subnet"
  region = "us-central1"
  ip_cidr_range="10.0.1.0/24"
  depends_on    = [google_compute_network.vpc-net]
  network= "gowtham-terraform-vpc"
}
resource "google_compute_firewall" "vpcf" {
  name    = "gowtham-terraform-firewall"
  network = "gowtham-terraform-vpc"
...
resource "google_compute_instance_template" "tmp1" {
  ...
  network_interface {
    network = "gowtham-terraform-vpc"
   subnetwork = "gowtham-terraform-subnet"
   access_config {
        }
  }
...
resource "google_compute_region_instance_group_manager" "mig-grp" {
  name               = "gowtham-mig"
  region             = "us-central1"
  base_instance_name = "gowtham-mig-ins"
  target_size        =  3
...
resource "google_compute_region_autoscaler" "wpex" {
  name   = "gowtham-region-autoscaler"
  region = "us-central1"
